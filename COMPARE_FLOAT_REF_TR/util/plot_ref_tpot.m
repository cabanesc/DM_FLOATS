function plot_ref_tpot(DATA_BASE,CONFIG,varargin)
% -========================================================
%   USAGE : plot_ref_tpot(DATA_BASE,CONFIG,varargin)
...
if ~isempty(thetitle)
 if isempty(strfind(thetitle,DATA_BASE))
        title(['Potential Temperature from ARGO and CTD reference databases']);
 else
        %cla(gca)
        ModeClim='auto';
        title(['Potential Temperature from ' DATA_BASE ' reference database']);
 end
else
...
for ibox=1:nbox % boucle sur toutes les boites
    ...
    for k=1:length(ip)
        ...
        plot(CTD.temp.data(ip(k),:)',CTD.pres.data(ip(k),:)','Color',s.MarkerFaceColor);
