...
c=varargin(2:2:end);
s = cell2struct(c,f,2);
accepted_options={'MarkerSize','MarkerFaceColor','MarkerSpec','MarkerLine','Isbest','Number_floats'};
accepted_Param={'','psal_mean','thedates','pres_mean','longitude','latitude','platform_number'};
...
if isfield(s,'Number_floats')==0
    s.Number_floats=[]; %default
end
...
if isfield(F,'psal')
    ...
    if strcmp(s.MarkerLine,'on')
        plot(F.thedates.data(ip),F.psal_mean.data(ip),'-m','LineWidth',1)
    end
    scatter(F.thedates.data(ip),F.psal_mean.data(ip),s.MarkerSize,['m' s.MarkerSpec])
    if isempty(Param)==0
        if strcmp(Param,'platform_number')
            F.(Param).data=str2num(F.(Param).data);
            if isempty(s.Number_floats)==0
            F.(Param).data=repmat(s.Number_floats(1),[1,length(F.(Param).data)]);
            end
        end
        scatter(F.thedates.data(ip),F.psal_mean.data(ip),s.MarkerSize,F.(Param).data(ip),s.MarkerSpec,'filled')
    else
       scatter(F.thedates.data(ip),F.psal_mean.data(ip),s.MarkerSize,s.MarkerFaceColor,s.MarkerSpec,'filled')
    end
    if strcmp(s.MarkerLine,'on')
        scatter(F.thedates.data(ip),F.psal_mean.data(ip),s.MarkerSize,['m' s.MarkerSpec])
    end
    %keyboard
%     F.platform_number.data
%     disp('please enter to continue')
%     pause
end