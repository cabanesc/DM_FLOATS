...
if ~isnan(F.longitude.data)&&~isnan(F.latitude.data)
    ...
    % maxlat=max(max(CTD.longitude.data),F.longitude.data)+2;
    ...
    MAP_VISU.max_lat = max(F.latitude.data,max(CTD.latitude.data));
    MAP_VISU.max_lon = max(F.longitude.data,max(CTD.longitude.data));
    MAP_VISU.min_lat = min(F.latitude.data,min(CTD.latitude.data));
    MAP_VISU.min_lon = min(F.longitude.data,min(CTD.longitude.data));
    ...
    %if PARAM.isarctic==0
    PARAM.figsize='large'
    plot_hist_loc(MAP_VISU,CTD,F,index_closest_date,PARAM);
    %else
    ...
    %     m_grid('yTick',[5:10:85]);
    %     m_grid('yTick',[5:1:90],'YTickLabel',[]);
    ...
    msg='nok';h1=[];h2=[];h3=[];hf=[];
    disp('Profile position is unknown')
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_hist_loc(MAP_VISU,CTD,F,index_closest_date,PARAM)

if PARAM.isarctic==0
    
    
    fct_pltmap_bathy(MAP_VISU.zone_visu,MAP_VISU.reso,MAP_VISU.proj);
    hold on
else
    hold on
    ...
    switch PARAM.figsize
        case 'large'
            pas_large=ceil(min(MAP_VISU.max_lat-MAP_VISU.min_lat+10,50)/5);
            m_grid('yTick',[5:pas_large:85]);
            m_grid('yTick',[5:1:90],'YTickLabel',[]);
        case 'small'
            pas_large=ceil(min(MAP_VISU.max_lat-MAP_VISU.min_lat+10,50)/2);
            m_grid('yTick',[5:pas_large:85],'Fontsize',8);
    end
...
switch PARAM.figsize
    ...
    case 'small'
        h1 = m_plot(F.longitude.data,F.latitude.data,'ro'); set(h1,'markersize',7,'markerfacecolor','r');
end
