...
%         points specified in the LAT and LONG vectors or pairs of vectors
%         (decimal degrees with positive indicating north/east). 
%         Forward and reverse bearings (degrees) are returned in AF, AR.
%
...
%         and N=2, or LAT=0...
%       PL (LODYC) 05/Nov/97
%         Lat and long can be matrices of 2 vectors.
...
if isempty(lat) | isempty(long) % sanity check -- D.B.
  return
end
...
if min(size(lat))==1,
     lat=lat(:); long=long(:);
elseif min(size(lat))>2, error('1 or 2 vectors only in lat,long'); 
elseif size(lat,2)==2, lat=lat'; long=long';
end;
...
lat(lat==0)=eps+lat(lat==0);   % geodesics stuff
