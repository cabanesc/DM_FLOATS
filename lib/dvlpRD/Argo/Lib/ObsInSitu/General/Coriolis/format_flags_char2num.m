...
% -========================================================
%   USAGE : Pr = flags_char_to_num(Co,iadj);
...
%if isempty(strfind(Co.obj,'ObsInSitu/Coriolis'))
   % error('format_flags_char2num not define for this type of structure')
%else
    fillval='FillValue_';
	...
	if isempty(findstr(oneChamp,'_qc'))==0            % test si il y a 'qc' dans le nom du champ
		...
		if thisiscar==1      % test si c'est une chaine de caractère
			...
			if isalphanum==1  % test si tableau alphanumerique
			    ...
			    Pr.(oneChamp).data=999.*ones(size(Co.(oneChamp).data)); % fillval numeriques
			    Pr.(oneChamp).type='double';
			    Pr.(oneChamp).(fillval)=999.;
				...
				if sum(isnofill)~=0 
				    ...
				    Pr.(oneChamp).data(i,isnofill)=flagnum;
