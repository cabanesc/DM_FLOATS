...
for k=[isort]
    ...
    if strcmp(strtrim(software),'COQC')
        software= 'COQC (real time tests) ';
    end
    ...
    % HISTORY QC_TEST
    hextests = squeeze(Co.history_qctest.data(k,profile_number,:))';
    %hextests
    %sum(isstrprop(hextests,'xdigit'))
    %length(hextests)
   % keyboard
    if sum(isstrprop(hextests,'xdigit')|isstrprop(hextests,'wspace'))==length(hextests)
      total = hex2dec(hextests);
      
      totala= total;
      % find the real time tests corresponding to the hex code
      add=[];
      if strcmp(software,'COQC (real time tests) ')
    ...
    disp('---------------------------------------------------')
    disp(['Not possible to decode the hexadecimal test (not an hexadecimal number):' hextests] )
   end 
end