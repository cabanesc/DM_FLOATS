...
configParam = {'CycleTime','DownTime','UpTime','ParkPressure','ProfilePressure','DeepProfileFirstFloat','IceDetection'};
...
% initialize
M.(configParam{iparam}) = NaN*zeros(DimM.n_missions.dimlength,1);
idF = find(findstr_tab(launch_config_parameter,['CONFIG_' configParam{iparam} '_degC']));
if isempty(idF)==0
    M.(configParam{iparam}) = repmat(Mf.launch_config_parameter_value.data(idF),DimM.n_missions.dimlength,1);
end
...
iparam = 7;
idF = find(findstr_tab(config_parameter,['CONFIG_' configParam{iparam} '_degC']));
if isempty(idF)==0
    M.(configParam{iparam}) = Mf.config_parameter_value.data(:,idF);
end
%--------------------------------------------------------------------------------------------------------------
