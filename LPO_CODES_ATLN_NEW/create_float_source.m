% ========================================================
%   USAGE :  create_float_source(flt_name,dacname)
%
...
%     dacname    (char) -- dac name  e.g.: 'coriolis'
%   OPTIONAL INPUT :
...
%  libargo
%  seawater
%  comparé a la version distribuée: appel de fontion libargo et ajout de dacname en argument 
...
float_netcdf_directory = [C.DIR_FTP dacname '/'];    % input files directory
float_matfile_directory = [C.DIR_DATA 'float_source/'];   % output files directory
...
if exist(mat_filename,'file')
    ...
    if isfield(s,'force')==0 % take into account the adjusted pressure if needed
        ...
        if mean(meanoutnan(thediff))~=0 && ~isfield(s,'force') % if PRES_ADJUSTED is different from PRES 
    ...
    for j=1:d1;
        if(isnan(date_greg(j,:))==0)
            ...
            if(mo<1|mo>12|day<1|day>31)
                ...
                dates(j)=yr+cal2dec(mo,day,hr,minut)./365;
            end
        else
            dates(j)=NaN;
        end
