...
%map_p_exclude=ceil(max(min(P)-map_p_delta,0));
%disp(' ')
%disp(['map_p_exclude (re-computed to avoid mapping of unnecessary levels):' num2str(map_p_exclude)])
%disp(' ')
...
if length( missing_profile_index )>0
    ...
    save(thefilename,'la_bhist_lat_all', 'la_bhist_long_all', 'la_bhist_dates_all', 'la_bhist_Z_all','index_each','index_un','Min_ptmp','float_data');
