...
function [ index ] = find_besthist( grid_lat, grid_long, grid_dates, grid_Z, LAT, LONG, DATES, Z, latitude_large, latitude_small, longitude_large, longitude_small, phi_large, phi_small, age, age_large, map_pv_use, max_casts,mask )
...
% change config 129 : add age_large when computing correlation_large
...
% use the mask to remove some reference data
mask_ref=interp2(mask.lon,mask.lat,mask.mask,grid_long,grid_lat);
mask_float=interp2(mask.lon,mask.lat,mask.mask,LONG,LAT);

index_ellipse = find( ellipse<1 &mask_ref==mask_float) ;
