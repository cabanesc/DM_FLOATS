...
%        m_proj('stereographic', 'lat', meanoutnan(LAT), 'long', 0, 'radius', max(LAT)+1- min(LAT)+1); 
%     else
%     m_proj('mercator','long', [min(LONG1)-1, max(LONG1)+1], 'lat', [min(LAT)-1, max(LAT)+1] );
%     end
    m_proj('mercator','long', [min(LONG1)-1, max(LONG1)+1], 'lat', [max(min(LAT)-1,-90), min(max(LAT)+1,90)] );% correction ccabanes : problem if LAT=90
