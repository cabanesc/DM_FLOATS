...
try
    ...
    breaks = [];
    max_breaks =4;
    %max_breaks = [-1 -1];   % 0 for linear trend and -1 for offset only!
    calseries = [ones(1,n)];
    %calseries = [ones(1,93) 2*ones(1,n-93)];    % example: split the time series at profile 33
    calseries = [ones(1,67) 2*ones(1,112-67) 0*ones(1,n-112)]; % example: ignore profile 34
