...
for i=1:n_seq
    ...
    k = length(calindex);
        ...
        unique_coord_float = coord_float(calindex,:); % ccabanes 07/11/2019
        ...
        if(isempty(pp)==0) % only proceed when there are valid levels ----
            ...
            covariance = build_cov(ten_PTMP,unique_coord_float,po_system_configuration);   % change config 129 :  vertical and horizontal covariances
