...
function plot_diagnostics_ow_figure9_1( pn_float_dir, pn_float_name, po_system_configuration,dacname )
...
xlabel('Longitude');
ylabel('Latitude');
title( strcat(title_floatname, ' ARGO +CTD REF : ', po_system_configuration.CONFIGURATION_FILE(1:end-4) ),'interpreter','non' );
...
legend(['2 x cal error','1 x cal error','1-1 profile fit (' strrep(po_system_configuration.CONFIGURATION_FILE(1:end-4),'_',' ') ')'], 'Location', 'SouthOutside','orientation','horizontal');
...
ylabel('\Delta S')
title( [' vertically-averaged salinity (PSS-78) additive correction \Delta S (red: config ' strrep(po_system_configuration.CONFIGURATION_FILE(1:end-4),'_',' ') '; red +: config 371)'] );
grid on
