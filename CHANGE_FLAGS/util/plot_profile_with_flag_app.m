...
function [thetitle]=plot_profile_with_flag_app(UIAXES,S,ParamX,ParamY,profile_number,varargin)
            ...
            if nprof>0&nprof<=size(S.(ParamX).data,1)
                ...
                if isfield(S,ParamY)  % abscisse
                    if isfield(S.(ParamY),'data') 
                        ...
                        plot(UIAXES,subX,level ,'Color',theLineColor,'LineWidth',p.LineWidth) ;
                        
                        hold(UIAXES)
                        grid(UIAXES)
						
                        xlabel(UIAXES,[upper(ParamX)],'interpreter','none')
                        ylabel(UIAXES,[upper(ParamY)],'interpreter','none')
                        set(gca,'Ydir','normal')
