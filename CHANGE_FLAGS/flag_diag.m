...
if isfield(s,'Config')==1
	...
	handles.DIR_PLOT=s.Config.DIR_PLOT;
	handles.NB_PROF=s.Config.NB_PROF;
else
...
if strcmp(parameters{choice},'Temperature')||strcmp(parameters{choice},'Salinity')||strcmp(parameters{choice},'ThetaS')||strcmp(parameters{choice},'Sigma0')||strcmp(parameters{choice},'Oxygene')
...
%     for k=1:r-d+1
    if ~isempty(min_x)|~isempty(max_x)|~isempty(min_y)|~isempty(max_y)
        ...
        elseif strcmp(parameters{choice},'ThetaS')
            ...
            % handles.F.psal_qc_new=handles.F.psal_qc;
            handles.F.psal_qc.data(k,isub)=str2num(new_flag);  
        elseif strcmp(parameters{choice},'Sigma0')
            ptemp = sw_ptmp(handles.F.psal.data,handles.F.temp.data,handles.F.pres.data,0);
			sigma0= sw_dens0(handles.F.psal.data,ptemp)-1000;
            isub=find(sigma0(k,:)<=max_x&sigma0(k,:)>=min_x&handles.F.pres.data(k,:)<=max_y&handles.F.pres.data(k,:)>=min_y);
            hold on
            
			
			
            plot(sigma0(k,isub),handles.F.pres.data(k,isub),[colormarker{str2num(new_flag)} '+'])
            % handles.F.psal_qc_new=handles.F.psal_qc;
            handles.F.psal_qc.data(k,isub)=str2num(new_flag); 			
...
if isfield(handles,'choice_previous')==1
 ...
 choice
	...
	if handles.choice_previous~=7&&handles.choice_previous~=13 % oxygene or oxygene adjusted
	...
	markerstr
		...
		elseif strcmp(parameters{choice},'Oxygene')
		 ...
		 elseif strcmp(parameters{choice},'Ptemp')
		   % keyboard
			...
			handles.F.ptemp=handles.F.temp; %init
			handles.F.ptemp_qc=handles.F.temp_qc; % init
		...
		elseif strcmp(parameters{choice},'Sigma0')
		   % keyboard
			...
			if isempty(handles.F2)==0
			n_prof2=size(handles.F2.temp.data,1);
			ptemp=sw_ptmp(handles.F2.psal.data,handles.F2.temp.data,handles.F2.pres.data,0);
			handles.F2.sigma0.data=sw_dens0(handles.F2.psal.data,ptemp)-1000;
			handles.F2.sigma0_qc.data=handles.F2.psal_qc.data;
			plot_profile_with_flag(handles.F2,'sigma0','pres',[1:n_prof2],'LineWidth',0.5,'Marker','none');
			end
			ptemp=sw_ptmp(handles.F.psal.data,handles.F.temp.data,handles.F.pres.data,0);
			handles.F.sigma0=handles.F.temp;% init
			handles.F.sigma0_qc=handles.F.temp_qc; % init
			handles.F.sigma0.data=sw_dens0(handles.F.psal.data,ptemp)-1000;
			handles.F.sigma0_qc.data=handles.F.psal_qc.data;
			thetitle= plot_profile_with_flag(handles.F,'sigma0','pres',[1:n_prof],'LineWidth',0.5,'Marker',markerstr);
			%thetitle= plot_profile_with_flag(handles.F,'ptemp','pres',[1:n_prof],'LineWidth',0.5,'Marker','none');
			title(thetitle)    
		    axes(handles.ax3)
			cla(gca)
		...
		elseif strcmp(parameters{choice},'Ptemp(Adjusted)')
		   % keyboard
			...
			handles.F.ptemp_adjusted=handles.F.temp_adjusted;
			handles.F.ptemp_adjusted_qc=handles.F.temp_adjusted_qc;
...
if length(thelist)>1
    ...
    k=[str2num(k)-handles.NB_PROF:str2num(k)+handles.NB_PROF];
    k(k<1)=1;
...
ii=strfind(handles.thetitle,'cycle');
cut_thetitle=handles.thetitle(ii:end);
if isempty(cut_thetitle)==0
   
   [r,t]=strtok(cut_thetitle,',');
   cut_thetitle=strrep(r,'cycle:','');
   cut_thetitle=strrep(cut_thetitle,'cycles:','');
   cut_thetitle=strrep(cut_thetitle,' ','');
   cut_thetitle=strrep(cut_thetitle,'-','_');
   cut_thetitle=strrep(cut_thetitle,'A','');

dirname=[handles.DIR_PLOT '/change_flag/' handles.floatname '/'];
if exist(dirname)==0
mkdir(dirname)
end
filename=[dirname handles.floatname '_changeflag_' cut_thetitle '.pdf']
print(handles.figure1,filename,'-dpdf')
end
% --- Executes during object creation, after setting all properties.
